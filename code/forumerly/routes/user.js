...
// Change upload logic to only accept image file types:
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    // Reject file if it is not an allowed type.
...
function adminRequired(req, res, next) {
  if (req.isAuthenticated()) {
    if (!req.user.admin) {
      req.flash(
        "error",
        "Only site administrators are permitted to visit that page.",
      );
      return res.redirect("back");
    }
    next();
...
function formatSingleObject(data) {
  ...
  data.relativeLastPostDate = moment(data.lastPostDate)
    .startOf("minute")
    .fromNow();
  if (data.replies) {
...
router
  ...
  .get("/user/:username", (req, res) => {
    ...
    mongo.db
      .collection("users")
      .findOne(
        { lcUsername: req.params.username.toLowerCase() },
        (err, result) => {
              ...
              .toArray((err, result) => {
                ...
                mongo.db
                  ...
                  .toArray((err, result) => {
                    ...
                    profile.postCount =
                      profile.threadsCreated + profile.repliesCreated;
                    if (result.length === 0) {
                    ...
                    res.render("profile", {
                      profile: profile,
                      repliesTab: req.query.tab === "replies",
                      viewerIsOwner: bool,
                    });
                  });
              });
  ...
  .post("/upload", loginRequired, upload.single("avatar"), (req, res) => {
    ...
    const ext = extensionByMime[req.file.mimetype]; // Determine correct extension.
    const safeFilename = req.user.username.replace(/[^a-zA-Z0-9_-]/g, "") + ext; // Sanitize username to avoid path issues.
    const finalPath = path.join(req.file.destination, safeFilename); // Build final file path.

    // Rename uploaded file to controlled filename.
    fs.rename(req.file.path, finalPath, (err) => {
      ...
      mongo.db
        ...
        .updateOne(
          ...
          { $set: { img: "/images/profileImages/" + safeFilename } }, // Store safe image path with extension.
          (err) => {
            if (err) throw err;
            res.redirect("back");
          },
  ...
  .post(
    "/upload/users",
    uploadUsers.single("upload-users"),
    async (req, res) => {
      try {
          ...
          newUsers.map(async (item) => {
            return new Promise(async (resolve, reject) => {
              ...
              if (user) {
                ...
                mongo.db.collection("users").insertOne(item, (err, result) => {
                  if (err) {
                    console.log(err);
                    reject(err);
                  } else {
